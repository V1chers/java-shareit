...
public class BookingController {
    ...
    @GetMapping
    public List<BookingDto> getBookings(@RequestParam(defaultValue = "ALL") String stateParam,
                                        @RequestHeader("X-Sharer-User-Id") int userId) {
    ...
    @GetMapping("/owner")
    public List<BookingDto> getOwnBookings(@RequestParam(defaultValue = "ALL") String stateParam,
                                           @RequestHeader("X-Sharer-User-Id") int userId) {
        State state = State.from(stateParam)
                .orElseThrow(() -> new ConditionsNotMetException("Передано неизвестное состояние: " + stateParam));
