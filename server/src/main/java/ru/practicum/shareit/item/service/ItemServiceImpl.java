...
public class ItemServiceImpl implements ItemService {
    ...
    private List<ItemCommentsDto> fillComments(List<Item> itemList, List<Comment> comments) {
        HashMap<Integer, List<Comment>> itemsComments = new HashMap<>();
        comments.forEach(comment -> {
            List<Comment> commentList = itemsComments.computeIfAbsent(comment.getItemId(), k -> new ArrayList<>());

            commentList.add(comment);
        });

        List<ItemCommentsDto> itemCommentsDtoList = new ArrayList<>();
        itemList
                .forEach(item -> {
                    List<Comment> commentList = itemsComments.get(item.getId());
                    ItemCommentsDto itemCommentsDto;

                    if (commentList == null) {
                        itemCommentsDto = ItemMapper.toItemCommentsDto(item);
                    } else {

                        itemCommentsDto = ItemMapper.toItemCommentsDto(item, itemsComments.get(item.getId()));
                    }

                    itemCommentsDtoList.add(itemCommentsDto);
                });
